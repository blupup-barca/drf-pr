...
def block_user():
    ...
    users_to_block = (
        CustomUser.objects.filter(
            last_login__isnull=False, last_login__lte=time_threshold, is_active=True
        )
        .exlude(is_staff=True)
        .exlude(is_superuser=True)
    )
    ...
    if blocked_users:
        ...
        for user in blocked_users:
            print(
                f"ID: {user['id']} | Email: {user['email']} | Последний вход: {user['last_login']}"
            )
    else:
